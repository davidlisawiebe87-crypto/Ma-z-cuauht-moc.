...
async function getCorn() {
  try {
    return await getCornYahoo();
  } catch (yahooError) {
    try {
      return await getCornStooq();
    } catch (stooqError) {
...
async function getCornYahoo() {
  const url = 'https://query1.finance.yahoo.com/v8/finance/chart/ZC=F?range=5d&interval=1d';
  const response = await fetchWithTimeout(url, { headers: { accept: 'application/json' } });
  const data = await response.json();
  const result = data?.chart?.result?.[0];
  const closes = result?.indicators?.quote?.[0]?.close?.filter(Number.isFinite) || [];
  if (!closes.length) throw new Error('Sin cierres de maíz');
  const currentRaw = closes.at(-1);
  const previousRaw = closes.at(-2) ?? currentRaw;
  const priceUsdBu = normalizeCornPrice(currentRaw);
  ...
  return {
    ...
    source: 'Yahoo Finance / CBOT continuo'
  };
}

async function getCornStooq() {
  const response = await fetchWithTimeout('https://stooq.com/q/l/?s=zc.f&i=d');
  const text = await response.text();
  const lines = text.trim().split(/\r?\n/).filter(Boolean);
  if (!lines.length) throw new Error('CSV vacío');
  const cells = lines.at(-1).split(',');
  const hasHeader = /symbol/i.test(lines[0]);
  let close;
  let date;
  if (hasHeader && lines.length > 1) {
    const headers = lines[0].split(',').map(s => s.trim().toLowerCase());
    const values = lines[1].split(',');
    close = Number(values[headers.indexOf('close')]);
    date = values[headers.indexOf('date')];
  } else {
    close = Number(cells[6] ?? cells[4]);
    date = cells[1];
  }
  if (!Number.isFinite(close)) throw new Error('Cierre inválido');
  return {
    priceUsdBu: normalizeCornPrice(close),
    changePct: 0,
    date: normalizeStooqDate(date),
    source: 'Stooq / CBOT continuo'
  };
}
...
function normalizeStooqDate(value) {
  ...
  if (/^\d{8}$/.test(text)) return `${text.slice(0,4)}-${text.slice(4,6)}-${text.slice(6,8)}T12:00:00Z`;
